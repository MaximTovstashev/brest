...
var Method = function(brest, description){
    ...
    self.onRequest = function(req, res){
        ...
        _.each(req.query,function(value, filter){
            if (self.filters && self.filters[filter]){
                ...
                if (self.filters[filter].replaceMe && (_.findIndex(self._replaceMe, value) > -1)) {
                    console.log(self._replaceMe);
            ...
            self.checkAuth(req, function (authPassed) {
                if (authPassed) {
                    self.handler(req, function (err, result, options) {
                        console.log('handler passed', self.description);
                        if (!_.isEmpty(self.description)
                            && _.isArray(self.description.screen.noAuth)
                            && _.isEmpty(req.user)) {

                            _.each(self.description.screen.noAuth, function(key){
    ...
    if (self._brest.getSetting('toobusy.interval')) {toobusy.maxLag = self._brest.getSetting('toobusy.interval')}
