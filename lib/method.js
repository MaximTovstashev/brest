...
class Method extends EventEmitter{
	...
	constructor(brest, description) {
		...
		const api_url = [];
		if (this._brest.getSetting('api_url.prefix')) api_url.push(this._brest.getSetting('api_url.prefix'));
		if (!this._brest.getSetting('api_url.unversioned')) api_url.push(`v${description.version}`);
