...
var Method = function(brest, description){
        ...
        if (self.filters) {
            ...
            _.each(req.query, function (value, filter) {
                ...
                if (self.filters[filter]) {
                    ...
                    if (self.filters[filter].toLowerCase === true) {
                        value = value.toLowerCase();
                    }

                    if (self.filters[filter].toUpperCase === true) {
                        value = value.toUpperCase();
                    }

                    if (self.filters[filter].toFinite === true) {
                        value = _.toFinite(value);
                    }

                    if (self.filters[filter].toInteger === true) {
                        value = _.toInteger(value);
                    }

                    if (self.filters[filter].toNumber === true) {
                        value = _.toNumber(value);
                    }

                    if (self.filters[filter].toBoolean === true) {
                        value = _.toString(value) !== 'false';
                    }

                    if (_.isFunction(self.filters[filter].transform)) {
                        value = self.filters[filter].transform(value);
                    }

                    //Convert value to array if received data is not an array already
                    if (self.filters[filter].toArray) {
                        if (!_.isArray(value)) {
                            value = value.split(_.isString(self.filters[filter].toArray) ? self.filters[filter].toArray : ',');
                        }
                    }
    ...
    self.checkFilterAlias = function(key, filter) {
