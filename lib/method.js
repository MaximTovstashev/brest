...
class Method extends EventEmitter {
  ...
  constructor(brest, description) {
    ...
    if (this._brest.getSetting('toobusy.maxLag')) {
      toobusy.maxLag = this._brest.getSetting('toobusy.maxLag');
    }
    if (this._brest.getSetting('toobusy.interval')) {
    ...
    if (tooBusyEnabled) {
      description.middle.push(tooBusyMiddleware);
    }
    ...
    if (description.allowCORS) {
      ...
      if (this.method.toLowerCase() !== 'get') {
        this._app.options(uri, cors({optionsSuccessStatus: code.HTTP_OK}));
      }
    }
  ...
  processAuthorizedRequest(authError, req, res) {
    ...
    if (!authError) {
      ...
      if (handlerPromise instanceof Promise || typeof handlerPromise.then === 'function') {
        handlerPromise.then((result)=>{
          const options = Method.detachOptions(result);
