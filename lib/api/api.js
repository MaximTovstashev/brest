...
function callHandler(req, res, handler, noAuth){
    if (noAuth || (req.user && req.user.FBID)){
        ...
        send(res,{message:"You must be authorized to use this method"},ec.RESOURCE_NOT_FOUND);
    }
...
//                            }
        ...
        if (!_.isUndefined(APIres[resource.uri][resource.method]))
            throw {message: "Duplicate method "+rd.method+" for "+debugURI, code: er.INTERNAL_SERVER_ERROR};
