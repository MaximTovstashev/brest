...
class Endpoint extends EventEmitter {
   ...
   */
    constructor(brest = required('brest'), description = required('description')) {
        ...
        if (!apiUrlSettings.unversioned) apiUrl.push(`v${description.version}`);
        apiUrl.push(description.noun);
        if (description.uri) apiUrl.push(description.uri);

        const uri = '/' + apiUrl.join('/');
    ...
    //Kick in cors middleware
      ...
      //Enable pre-flight "OPTIONS" request support
        ...
        if (description.bodyParser) {
            require('./middleware/bodyparser').initEndpoint(this.brest, description);
        }
