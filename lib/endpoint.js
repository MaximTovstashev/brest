...
class Endpoint extends EventEmitter {
   ...
   */
    constructor(brest = required('brest'), description = required('description')) {
        ...
        const uri = (apiUrl[0].charAt(0) === '/' ? '' : '/') + urljoin(...apiUrl);
        ...
        const corsSettings = brest.getSetting('cors', {optionsSuccessStatus: code.HTTP_OK});

        //Kick in cors middleware
        if (this.$fields.allowCORS) {
            this.$fields.middle.push(cors(corsSettings));

            //Enable pre-flight "OPTIONS" request support
            if (this.verb.toUpperCase() !== c.VERB_GET) {
                this.app.options(uri, cors(corsSettings));
            }
        }
   ...
   */
    _processError(error = required('error'), res = required('res'), verbose = true) {
        async.each(this.extensions, (extension, next_extension) => {
            if (extension.endpoint && extension.endpoint.error) {
                extension.endpoint.error(this, error, (extension_err, result) => {
                    error = result || error;
                    next_extension(extension_err);
                });
            } else {
                next_extension();
            }
        }, (extension_err) => {
            const err = extension_err ? generateReturnableError(extension_err) : generateReturnableError(error);
            if (verbose) {
