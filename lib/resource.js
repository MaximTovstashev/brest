...
var notSupported = function(req, res){
...
var Resource = function(app, description){
    ...
    _.each(description.resources, function(resource){
        ...
        if (!_.isNumber(resource.version) || Math.round(resource.version) != resource.version || resource.version<1)
            throw {message: "Incorrect \""+description.noun+"\" resource version \"" + resource.version + "\": only positive integers are valid"};

        resource.noun = description.noun;

        var method = new Method(this._app, resource);
    ...
    _.each(this.resourceURI, function(resourse, uri){
        ...
        for (var i=0; i < strayMethods.length; i++){
            this._app[strayMethods[i].toLowerCase()](uri, notSupported);
        }
    }, this);
};
