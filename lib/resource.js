...
class Resource extends EventEmitter {
   ...
   */
    constructor(brest, description) {
        ...
        logger = intel.getLogger(`resource.${this.noun}`);
        ...
        if (description.resources) {
            logger.warn('WARNING: deprecated "resources" API description property. Use "endpoints" instead'.yellow);
       ...
       */
            ...
            let endpointsToInitialize = description.endpoints.length || 0; //Specificly set to {int}0
            ...
            _.each(this.resourceURI, (resourse, uri) => {
                ...
                const stray_verbs = _.difference(verbs, used_verbs);

                const strayVerbEndpoint = this.createStrayVerbEndpoint(used_verbs.join());
                stray_verbs.forEach((stray_verb) => {
   ...
   * @returns {Function}
   */
    createStrayVerbEndpoint(supportedString) {
        return (req, res) => {
