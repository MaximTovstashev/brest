...
var Brest = function(settings){
    ...
    self.getSetting = function(key, defaultValue) {
        ...
        if (_.isObject(s) && _.isObject(defaultValue)) return _.defaultsDeep(s, defaultValue);
        else return s;
    };
    ...
    self._emitCounterEvent = function(key, value) {
        if (self.settings.emitCounterEventOn[key] && _.indexOf(self.settings.emitCounterEventOn[key], self._counters[key]) > -1) {
            var eventData = {};
            eventData[key] = self._counters[key];
    ...
    //Starting the server
    self._server = http.createServer(self._app).listen(self.getSetting('server.port','8080'), function(err){
        if (err) {
            self.emit('error', err);
            console.log(err);
        } else {
