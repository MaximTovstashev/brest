...
class Brest extends EventEmitter {
  ...
  constructor(settings = {}, externalExpress, externalApp) {
    super();

    this.__dirname = settings.basePath || path.dirname(require.main.filename);
    ...
    this._express = externalExpress || express;
    this._app = externalApp || this._express();
    ...
    if (_.isFunction(this.getSetting(c.EVENT_AFTER_STATIC_INIT))) {
      this.getSetting(c.EVENT_AFTER_STATIC_INIT)(this.express, this.app);
    }
