...
class Brest extends EventEmitter {
  ...
  constructor(settings) {
    ...
    logger = intel.getLogger('brest.main');
    /**
    ...
    async.waterfall([
      ...
      (next) => {
        this._server = http.createServer(this.app).listen(this._port, (err) => {
          ...
          logger.info('Brest instance is down'.magenta);
          this.emit(c.EVENT_CLOSED);
        });
      }
    ], (err) => {
      if (err) {
        logger.error('!!!Error : <' + require('util').inspect(err, {depth: 5}) + '>'.red);
        return this.emit(c.EVENT_ERROR, err);
      }
      this._ready = true;
      this.emit(c.EVENT_READY);
      const server_name = settings.application || 'Unnamed';
      logger.info(`${server_name} server is now listening on port ${this._port}`.green);
    });
  ...
  attach(key, attachment) {
    if (this._attachments[key]) logger.warn(`WARNING: Already attached to: ${key}`.yellow);
  ...
  attachment(key) {
    if (this._attachments[key]) {
      ...
      logger.error(`ERROR: Attachment not found: ${key}`.red);
    }
  }
  ...
  bindFile(fileName, callback) {
    if (_.isString(fileName)) {
      ...
      logger.error(`Can't use ${fileName} as a legit path`.red);
  ...
  bindPath(apiPath, callback, suppressEventEmission = false) {
    ...
    if (_.isArray(apiPath)) {
      ...
      fs.readdir(apiPath, (err, resources) => {
        if (err) {
          logger.error('Failed to read path', apiPath);
          return callback(err);
        }
  ...
  _proceedWithExtensions(extensions) {
    logger.info('Initializing Brest extensions...');
    async.eachSeries(extensions, (extension, next_extension) => {
      this.ext(extension, next_extension);
    }, (err) => {
      if (err) {
        logger.error('Initializing Brest extensions failed with error'.red);
        logger.error(err);
        this.emit(c.EVENT_ERROR, err);
      } else {
        logger.info('Initializing Brest extensions: SUCCESS'.green);
   ...
   */
  getApp() {
    logger.warn('WARNING: Deprecated function Brest.getApp() use Brest.app instead'.yellow);
    return this._app;
  }
   ...
   */
  getExtensions() {
    logger.warn('WARNING: Deprecated function Brest.getExtensions() use Brest.extensions instead'.yellow);
   ...
   */
  getPort() {
    logger.warn('WARNING: Deprecated function Brest.getPort() use Brest.port instead'.yellow);
    return this._port;
  }
  ...
  close(callback) {
    if (!this._closing) {
      this._closing = true;
      logger.info('Shutting down Brest instance'.yellow);
