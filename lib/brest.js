...
class Brest extends EventEmitter {
  ...
  constructor(parameters = {}, extensions = []) {
    ...
    //bREST extensions
    this._extensions = [];
    this._preloadedExtensions = extensions;
    ...
    if (_.isFunction(this.getSetting(c.EVENT_BEFORE_STATIC_INIT))) {
      this.getSetting(c.EVENT_BEFORE_STATIC_INIT)(this);
    }

    if (_.isArray(extensions)) {
      extensions.forEach(extension => {
        if (_.isFunction(extension[c.EVENT_BEFORE_STATIC_INIT])) {
          extension[c.EVENT_BEFORE_STATIC_INIT](this);
        }
      });
    } else {
      log.warn('Expected extensions to be an array');
    }
    ...
    if (this.getSetting('static')) {
      ...
      if (s.favicon) this.app.use(s.favicon(this.__dirname + s.favicon));
      this.app.use(this.express.static(path.join(this.__dirname, s.public || DEFAULT_PUBLIC_DIR)));
    ...
    if (_.isFunction(this.getSetting(c.EVENT_AFTER_STATIC_INIT))) {
      this.getSetting(c.EVENT_AFTER_STATIC_INIT)(this);
    }

    if (_.isArray(extensions)) {
      extensions.forEach(extension => {
        if (_.isFunction(extension[c.EVENT_AFTER_STATIC_INIT])) {
          extension[c.EVENT_AFTER_STATIC_INIT](this);
        }
      });
    } else {
      log.warn('Expected extensions to be an array');
   ...
   * @private
   */
  _proceedWithExtensions(extensions = required('extensions')) {
   ...
   * @returns {Brest}
   */
  use(extensions = required('extensions')) {
    if (!_.isArray(extensions)) {
      this.emit(c.EVENT_ERROR, 'Expected array of extension objects in Brest.use()');
      return this;
    }
    extensions = this._preloadedExtensions.concat(extensions);
