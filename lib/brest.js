...
class Brest extends EventEmitter {
    ...
    async.waterfall([
      ...
      (next) => {
        ...
        if (this.getSetting('static')) {
          ...
          if (static_settings.favicon) this.app.use(static_settings.favicon(path.join(this.__dirname, static_settings.favicon)));
      ...
      //Binding API routes
      (next) => {
        if (this.getSetting('apiPath') && !this.getSetting('skipApiPathScan')) {
          log.info('go for binding paths');
          return this.bindPath(this.getSetting('apiPath'), next, this.getSetting('suppressEventsForSettingsAPI', false));
        } else {
          log.info('skip binding paths');
          return next();
        }
      },
      ...
      (next) => {
        log.info('>>> about to emit ready');
        this._ready = true;
        this.emit(c.EVENT_READY);
        log.info('>>> passed trough emit stage');
  ...
  bind(description = required(description), callback = required(callback)) {
    if (_.isArray(description)) {
      return async.each(description, (descriptionObject, nextDescription) => {
        this.bind(descriptionObject, nextDescription);
      }, callback);
    }
  ...
  bindFile(fileName = required('filename'), callback = required('callback')) {
    if (_.isArray(fileName)) {
      async.eachSeries(fileName,
        (singleFileName, nextFileName) => {
          this.bindFile(singleFileName, nextFileName);
        }, callback);
    }
  ...
  getSetting(key, defaultValue) {
    const value = _.get(this.settings, key);
    return defaultValue ? _.defaultsDeep(value, defaultValue) : value;
  }
